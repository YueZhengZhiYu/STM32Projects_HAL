 ...
 * @description: 在OLED特定位置开始显示16X16汉字 (修正版：适配连续32字节存储结构)
 * @param {uint8_t} x: 起始横坐标 (0~127)
 * @param {uint8_t} y: 起始页地址 (0~6, 因为需要占用y和y+1两页)
 * @param {uint8_t} no: 汉字在数组中的索引号
 * @param {uint8_t} Color_Turn: 0正常显示 1反色显示
 * @return {*}
 */
void OLED_ShowCHinese(uint8_t x, uint8_t y, uint8_t no, uint8_t Color_Turn)
{
    ...
    // 2. 发送上半部分的 16 个字节 (数据的前16个字节)
    for (i = 0; i < 16; i++)
    {
        if (Color_Turn)
            OLED_WR_DATA(~Hzk[no][i]); // 反色显示
        else
            OLED_WR_DATA(Hzk[no][i]);  // 正常显示
    }

    // 3. 定位光标到下半部分 (第 y+1 页)
    OLED_Set_Pos(x, y + 1);
    
    // 4. 发送下半部分的 16 个字节 (数据的后16个字节)
    for (i = 16; i < 32; i++)
    {
        if (Color_Turn)
            OLED_WR_DATA(~Hzk[no][i]);
        else
            OLED_WR_DATA(Hzk[no][i]);
    }
}
