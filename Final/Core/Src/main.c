...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define TEMP_ALARM_HIGH         28.0f
#define TEMP_ALARM_LOW          27.0f
#define HUM_ALARM_HIGH          80.0f
#define HUM_ALARM_LOW           75.0f
...
#define PID_KP_TEMP             2500.0f
#define PID_KI_TEMP             500.0f
#define PID_KD_TEMP             100.0f
#define PID_SETPOINT_TEMP       ((TEMP_ALARM_HIGH + TEMP_ALARM_LOW) / 2.0f)
#define PID_KP_HUM              1500.0f
#define PID_KI_HUM              100.0f
#define PID_KD_HUM              50.0f
#define PID_SETPOINT_HUM        77.0f
...
static uint16_t PID_FanControl(float temp, float hum)
{
    ...
    if (temp < PID_SETPOINT_TEMP - 1.5f) out_temp = 0.0f;
    else if (temp <= PID_SETPOINT_TEMP && out_temp < 1000.0f) out_temp = 0.0f;

    if (hum < PID_SETPOINT_HUM - 5.0f) out_hum = 0.0f;
    else if (hum <= PID_SETPOINT_HUM && out_hum < 1000.0f) out_hum = 0.0f;

    if (temp > PID_SETPOINT_TEMP && out_temp < 8000.0f) out_temp = 8000.0f;
    if (hum > PID_SETPOINT_HUM && out_hum < 8000.0f)    out_hum  = 8000.0f;
...
static void UpdateOLED_Screen1(void)
{
    ...
    OLED_ShowString(0, 4, "A236112", 16, 0);
	OLED_ShowCHinese(56, 4, 17, 0);
    OLED_ShowCHinese(72, 4, 18, 0);
    OLED_ShowCHinese(88, 4, 19, 0);
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (tick - last_sensor_tick >= SENSOR_INTERVAL_MS)
    {
        ...
        {
            float new_temp = dht_data.temperature * 0.92f;
        ...
        sprintf(usart_tx_buf, "T:%.1f,H:%.1f\r\n", current_temp, current_hum);
